...
def repeat(LANGUAGE,numE) :
    ...
    history = model.fit(training_data_generator, epochs=numE, verbose=2, steps_per_epoch=500)
    model.evaluate(test_data_generator)
    print("Finished" + LANGUAGE)
    model.save(LANGUAGE + str(numE)  + 'model.h5')
num = 10
while(num < 1000000):
    repeat("Python",num)
    repeat("Java",num)
...
#    result = np.argmax(model.predict(X_test[idx:idx+1]))
 #   print('Prediction: ', result ,' Char: ', chr(character_curated[result]))
